...
class ItemEntryDialog(QDialog):
    def __init__(self, parent, items, units, existing=None):
        ...
        form.addRow("Amount:", self.amount_lbl)

        setup_enter_nav(self, [
            self.item_cb, self.qty, self.unit_cb, self.rate,
    ...
    def _calc(self):
        amt = self.qty.value() * self.rate.value()
        self.amount_lbl.setText(format_inr(amt))
...
""")
        ...
        items_hdr.addWidget(items_lbl); items_hdr.addStretch(); items_hdr.addWidget(add_item_btn)
        root.addLayout(items_hdr)
        self._invoice_items = []
        self.items_table = QTableWidget(0, 9)
        self.items_table.setHorizontalHeaderLabels(
            ["Item", "Qty", "Unit", "Rate (\u20b9)", "CGST", "SGST", "IGST", "Amount (\u20b9)", ""])
        self.items_table.horizontalHeader().setSectionResizeMode(
            0, QHeaderView.ResizeMode.Stretch)
        for _c in range(1, 8):
            self.items_table.horizontalHeader().setSectionResizeMode(
                _c, QHeaderView.ResizeMode.ResizeToContents)
        self.items_table.horizontalHeader().setSectionResizeMode(8, QHeaderView.ResizeMode.Fixed)
        self.items_table.setColumnWidth(8, 40)
        ...
        add_tax_btn = QPushButton("+ Add Ledger")
        add_tax_btn.setStyleSheet(
        ...
        add_tax_btn.clicked.connect(lambda: self._add_tax_row())
        tax_hdr_lay.addWidget(tax_title)
        tax_hdr_lay.addStretch()
        tax_hdr_lay.addWidget(add_tax_btn)
        ...
        root.addWidget(self._tax_container)
    ...
    def _add_tax_row(self, placeholder="-- Select Ledger --", auto_fill=False):
        ...
        self._tax_v_lay.addWidget(row_w)
    ...
    def _refresh_items_table(self):
        ...
        for i, d in enumerate(self._invoice_items):
            ...
            self.items_table.setItem(r, 4, QTableWidgetItem(format_indian_number(d.get('cgst', 0))))
            self.items_table.setItem(r, 5, QTableWidgetItem(format_indian_number(d.get('sgst', 0))))
            self.items_table.setItem(r, 6, QTableWidgetItem(format_indian_number(d.get('igst', 0))))
            self.items_table.setItem(r, 7, QTableWidgetItem(format_indian_number(d['amount'])))
            
            # Delete button (at column index 8)
            del_btn = QPushButton()
            del_btn.setIcon(get_icon("frontend/assets/icons/trash.svg", "#c62828"))
            del_btn.setIconSize(QSize(16, 16))
            ...
            self.items_table.setCellWidget(r, 8, del_btn)
    ...
    def _refresh_totals(self):
        ...
        self.subtotal_lbl.setText(f"Sub-Total: {format_inr(subtotal)}")
